...
export async function createChannel(input: { channelName: string; baseUrl: string; apiFormat: number; providerKey: string }): Promise<Channel> {
...
export async function updateChannelName(channelId: number, channelName: string): Promise<Channel> {
  const response = await callAdminRpc(() => channelClient.updateChannelName({ channelId, channelName }));
  return requireChannel(response.channel);
}

export async function updateChannelStatus(channelId: number, status: number): Promise<Channel> {
  const response = await callAdminRpc(() => channelClient.updateChannelStatus({ channelId, status }));
  return requireChannel(response.channel);
}

export async function updateChannelBaseUrl(channelId: number, baseUrl: string): Promise<Channel> {
  const response = await callAdminRpc(() => channelClient.updateChannelBaseURL({ channelId, baseUrl }));
  return requireChannel(response.channel);
}

export async function updateChannelApiFormat(channelId: number, apiFormat: number): Promise<Channel> {
  const response = await callAdminRpc(() => channelClient.updateChannelAPIFormat({ channelId, apiFormat }));
  return requireChannel(response.channel);
}
