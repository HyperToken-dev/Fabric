...
func (a googleUsageAdapter) ProcessInteractionStreamingResponse(ctx context.Context, decodedBody []byte, info UsageContext) error {
	if info.ModelID == 0 {
		return fmt.Errorf("missing resolved model id for google streaming usage: key_id=%d, channel_id=%d, model=%q", info.KeyID, info.ChannelID, info.Model)
	}
	...
	zap.L().Info("google streaming usage log inserted",
		zap.Int32("key_id", info.KeyID),
		zap.Int32("channel_id", info.ChannelID),
		zap.Int32("model_id", info.ModelID),
		zap.String("model", info.Model),
